...
data class CalendarDayData(
    ...
    companion object {
        ...
        fun fromDomainModel(plannedMedicinesItems: List<PlannedMedicineItem>): CalendarDayData {
            val morningItems = getMorningItems(plannedMedicinesItems)
            val afternoonItems = getAfternoonItems(plannedMedicinesItems)
            val eveningItems = getEveningItems(plannedMedicinesItems)
            return CalendarDayData(
                morningItems = mapItemsToItemsData(morningItems),
                afternoonItems = mapItemsToItemsData(afternoonItems),
                eveningItems = mapItemsToItemsData(eveningItems)
            )
        }
        ...
        private fun mapItemsToItemsData(items: List<PlannedMedicineItem>): List<PlannedMedicineItemData> {
            return items.map { item ->
                PlannedMedicineItemData.fromDomainModel(item)
            }
        }
    }
}